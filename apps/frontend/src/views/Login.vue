<template>
    <div class="container">
        <div class="login-form">
            <h1>Welcome back!</h1>
            <p>Login to continue with the dashboard! 🙃</p>
            ...
            <Input placeholder="Password please..." icon="solar:key-minimalistic-bold" type="password"
                v-model="PasswordRef" />
            <Button icon="solar:forward-bold" icon-position="right" @click="Login()">Continue To Dashboard</Button>
            <span class="error">{{ ErrorRef }}</span>
            <RouterLink to="/register">Already not have an account??</RouterLink>
        </div>
    </div>
</template>
